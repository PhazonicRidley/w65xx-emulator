...
fn adc_test() {
    ...
    // PC set to 0xfff1, operand is at address 0xfff2, which has value of 2, moving PC forward a single byte.
    cpu.program_counter.increment(0);
    cpu.accumulator_cell.borrow_mut().value = 1;

    // Programmer is responsible for clearing the carry flag before adding
    ...
    // Verify
    assert_eq!(cpu.accumulator_cell.borrow().value, 3);
}
...
fn sbc_test() {
    ...
    // PC set to 0xffef, operand is at 0xfff0
    cpu.program_counter.increment(0xee);
    cpu.accumulator_cell.borrow_mut().value = 0x50;

    // Programmer is responsible for setting the carry flag to "complete" 2s compliment.
    ...
    // Verify
    assert_eq!(cpu.accumulator_cell.borrow().value, 0x60);
}
...
fn sbc_borrow_test() {
    ...
    // PC set to 0xff6f, operand is at 0xff70
    cpu.program_counter.increment(0x6e);
    cpu.accumulator_cell.borrow_mut().value = 0xd0;
...
fn bitwise_and_test() {
    // Set up
    let mut cpu = alu_test_setup();
    cpu.accumulator_cell.borrow_mut().value = 3;

    // Execute
    cpu.bitwise_and(&AddressingModes::Immediate);

    // Verify
    assert_eq!(cpu.accumulator_cell.borrow().value, 1);
}
...
fn bitwise_or_test() {
    // Set up
    let mut cpu = alu_test_setup();
    cpu.accumulator_cell.borrow_mut().value = 2;
...
fn bitwise_exclusive_or_test() {
    // Set up
    let mut cpu = alu_test_setup();
    cpu.accumulator_cell.borrow_mut().value = 0;
...
fn arithmetic_shift_lift_test() {
    // Setup
    let mut cpu = alu_test_setup();
    cpu.accumulator_cell.borrow_mut().value = 0x81;
    ...
    assert_eq!(cpu.accumulator_cell.borrow().value, 2);
}
...
fn rotate_left_test() {
    ...
    // Verify
    assert!(!cpu.processor_status_flags.check_flag(StatusFlags::Carry));
    assert_eq!(cpu.accumulator_cell.borrow().value, 0x7);
}
...
fn rotate_right_test() {
    // Setup
    let mut cpu = alu_test_setup();
    cpu.accumulator_cell.borrow_mut().value = 0x83;
    ...
    // Verify
    assert!(cpu.processor_status_flags.check_flag(StatusFlags::Carry));
    assert_eq!(cpu.accumulator_cell.borrow().value, 0xa0);
}
...
fn increment_test() {
    ...
    assert_eq!(cpu.x_cell.borrow().value, 2);
...
fn bit_test() {
    // Setup
    let mut cpu = alu_test_setup();
    cpu.accumulator_cell.borrow_mut().value = 0xff;
